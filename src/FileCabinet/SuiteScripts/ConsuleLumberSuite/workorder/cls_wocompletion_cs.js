...
define([
    ...
    /**
     * Load work order data for reference during completion using search
     */
    const loadWorkOrderData = (workOrderId) => {
        try {
            ...
            // Use search to get work order line data (client-safe)
            ...
            woSearch.run().each((result) => {
                workOrderData.lines.push({
                    ...
                    theoreticalBF: parseFloat(result.getValue(LINE_FIELDS.THEORETICAL_BF)) || 0,
                    calculatedBF: parseFloat(result.getValue(LINE_FIELDS.CALCULATED_BF)) || 0,
                    yieldPct: parseFloat(result.getValue(LINE_FIELDS.YIELD_PCT)) || 85
                });
                return true;
            });
        ...
        try {
            ...
            try {
                rec.setValue({
                    ...
                    value: roundTo(totalTheoreticalBF, PRECISION.BF),
    ...
    const handleQuantityChange = (rec, line) => {
        ...
        try {
            rec.setCurrentSublistValue({
                ...
                value: roundTo(wasteBF, PRECISION.BF),
                ignoreFieldChange: true
            });
        } catch (e) {
            // Field may not exist
        ...
        try {
            rec.setCurrentSublistValue({
                ...
                value: roundTo(actualQty, PRECISION.BF),
                ignoreFieldChange: true
            });
        } catch (e) {
            // Field may not exist
        ...
        try {
            rec.setCurrentSublistValue({
                ...
                value: roundTo(woLine.theoreticalBF, PRECISION.BF),
                ignoreFieldChange: true
            });
        } catch (e) {
            // Field may not exist
    ...
    const showYieldWarning = (actualPct, expectedPct) => {
        try {
            const msg = message.create({
                title: 'Yield Warning',
                message: `Recovery rate (${roundTo(actualPct, 1)}%) is below expected yield (${expectedPct}%). Please verify quantities or select a waste reason.`,
    ...
    const updateYieldSummary = (rec) => {
        try {
            ...
            // Update summary fields (silent fail if not present)
            try {
                rec.setValue({ fieldId: 'custpage_total_theoretical_bf', value: roundTo(totalTheoreticalBF, PRECISION.BF), ignoreFieldChange: true });
                rec.setValue({ fieldId: 'custpage_total_actual_bf', value: roundTo(totalActualBF, PRECISION.BF), ignoreFieldChange: true });
                rec.setValue({ fieldId: 'custpage_total_waste_bf', value: roundTo(totalWasteBF, PRECISION.BF), ignoreFieldChange: true });
                rec.setValue({ fieldId: 'custpage_overall_yield_pct', value: roundTo(overallYieldPct, PRECISION.PERCENTAGE), ignoreFieldChange: true });
            } catch (e) {
            ...
            if (woLine) {
                ...
                if (wasteBF > woLine.calculatedBF * 0.5) {
                    const proceed = confirm(
                        `Waste (${roundTo(wasteBF, 2)} BF) exceeds 50% of planned consumption. ` +
    ...
    const saveRecord = (context) => {
        ...
        try {
            ...
            if (totalTheoreticalBF > 0) {
                const overallYield = (totalActualBF / totalTheoreticalBF) * 100;

                if (overallYield < 50) {
                    const proceed = confirm(
                        `Overall yield is very low (${roundTo(overallYield, 1)}%). ` +
